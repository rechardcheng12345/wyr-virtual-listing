...
import { keyFromUrl, getPresignedDownloadUrl } from '../services/s3';
...
function buildEmailHtml(rows: { name: string; url: string | null }[]): string {
...
router.post('/', async (req: Request, res: Response) => {
  try {
    ...
    const listings: any[] = rows as any[];
